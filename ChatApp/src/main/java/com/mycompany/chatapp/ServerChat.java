    ...
    class socketConnexion implements Runnable {
        ...
        public void run() {
            try {
                ...
                boolean ok = false;
                int count;
                ObjectChat wrongPseudo = new ObjectChat(LocalTime.now().format(dateFormat), "Ce pseudo est deja pris", "Server");
                //Demande de pseudo:
                while(!ok) {
                    ...
                    if(!pseudo.toLowerCase().isEmpty()) {//test si il n'est pas vide
                        //ajouter un test si il contient bien uniquement des lettres;
